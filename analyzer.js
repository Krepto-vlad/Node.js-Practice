...
async function analyzeLogs(typeFilter) {
  ...
  try {
    ...
    if (e.code === "ENOENT") {
      console.error(
        "Logs folder not found. Please run the log generator FIRST! :("
      );
    } else {
...
function main() {
  ...
  if (args.includes("--type")) {
    ...
    if (!LOG_TYPES.includes(typeFilter)) {
      console.error(
        `Invalid type for filtering. Use one of: ${LOG_TYPES.join(", ")}.`
      );
      printHelp();
      process.exit(1);
    }
  ...
  console.log(
    "Log Analyzer started! If you want to see all available commands, run: npm run analyze -- --help \n"
  );
